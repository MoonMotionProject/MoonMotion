...
public class TerrainResponse : SingletonBehaviour<TerrainResponse>
{
	...
	private int firstRaycastedLayer()
	{
		...
		RaycastHit[] raycastHitsFound = Physics.RaycastAll(raycastingPosition, Direction.down.asVectorRelativeTo(bodyTransform), Mathf.Infinity, Physics.DefaultRaycastLayers);		// get all raycast hits for raycasting from the player's body relatively downward
	...
	public bool withinDistanceToTerrainBelow_(float maxDistanceBelow)
	{
		...
		RaycastHit[] raycastHitsFound = Physics.RaycastAll((raycastingPosition + new Vector3(0f, raycastingRaise, 0f)), Direction.down.asVectorRelativeTo(bodyTransform), Mathf.Infinity, Physics.DefaultRaycastLayers);		// get all raycast hits for raycasting from the player's body relatively downward
		if (raycastHitsFound.any())		// if at least one raycast hit was found
		{
			// determine the nearest raycast hit found's: hit distance, hit //
			float nearestRaycastHitDistance = Mathf.Infinity;		// initialize the nearest raycast hit's distance
