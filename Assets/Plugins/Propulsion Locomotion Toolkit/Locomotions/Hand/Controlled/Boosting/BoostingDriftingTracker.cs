...
public class BoostingDriftingTracker : PlayerDependencyTracker
{
	// variables //

	
	// variables for: instancing //
	private static BoostingDriftingTracker singleton;		// connection - automatic: the singleton instance of this class

	// variables for: tracking whether the player was last boosting drifting within a given time ago //
	public static float timePlayerWasLastBoostingDrifting = -Mathf.Infinity;		// tracking: the time the player was last boosting drifting – initialized to negative infinity as a flag that the player has never been boosting drifting
	...
	{
		return ((Time.time - timePlayerWasLastBoostingDrifting) <= timeAgo);
	}
	...
	protected override void Update()
	{
		...
		if (boostingDrifting())
		{
			...
			timePlayerWasLastBoostingDrifting = Time.time;
		}
	}
}