...
public class Hovering : MonoBehaviour
{
	...
	private float startingLocalPositionY = 0f;		// tracking: the starting local position y coordinate of this object (defaulted to zero as a fallback)
	...
	private void Start()
	{
		// track the object's starting local position y coordinate //
		startingLocalPositionY = transform.localPosition.y;

		// calculate the total time offset //
		totalTimeOffset = timeOffset + Random.Range(randomTimeOffsetMin, randomTimeOffsetMax);
	}
	...
	private void Update()
	{
		...
		float newLocalPositionY = startingLocalPositionY + (radius * Mathf.Sin((Time.time + totalTimeOffset) * frequencyFactor));
